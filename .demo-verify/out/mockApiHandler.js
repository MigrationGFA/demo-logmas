...
exports.DEMO_PRESET_USERS = void 0;
exports.getEffectiveServiceFee = getEffectiveServiceFee;
exports.handleMockApiRequest = handleMockApiRequest;
/* eslint-disable @typescript-eslint/no-explicit-any */
const lga_config_1 = require("@/config/lga.config");
const services_config_1 = require("@/config/services.config");
const store_1 = require("./store");
const applicationsStore_1 = require("./applicationsStore");
const apiPublicCertificate_1 = require("@/services/apiPublicCertificate");
// ==========================================
...
function getTreasurerFeeOverrides() {
...
function saveTreasurerFeeOverride(serviceId, amount, status) {
    if (typeof window === "undefined")
        return;
    try {
...
function readTabScheduleFee(serviceId) {
    if (typeof window === "undefined")
        return null;
    try {
        const keys = [
            `${lga_config_1.LGA_CONFIG.identity.id}_certificate_fees`,
            `${lga_config_1.LGA_CONFIG.identity.id}_levy_permit_fees`,
        ];
        for (const key of keys) {
            ...
            if (!Array.isArray(parsed))
                continue;
            const row = parsed.find((r) => r.serviceId === serviceId ||
                r.service === serviceId ||
...
function getEffectiveServiceFee(serviceId) {
    const srv = (0, services_config_1.getServiceById)(serviceId) || services_config_1.DEFAULT_SERVICES.find((s) => s.id === serviceId) || services_config_1.DEFAULT_SERVICES[0];
    const defaultAmount = Number(srv?.defaultFee ?? srv?.fee ?? 3500);
    const override = getTreasurerFeeOverrides()[serviceId];
    if (override)
        return { amount: override.amount, status: override.status, updatedAt: override.updatedAt };
    const tabFee = readTabScheduleFee(serviceId);
    if (tabFee && tabFee > 0)
        return { amount: tabFee, status: "ACTIVE", updatedAt: srv?.updatedAt || new Date().toISOString() };
    return { amount: defaultAmount, status: "ACTIVE", updatedAt: srv?.updatedAt || new Date().toISOString() };
}
function serviceWithEffectiveFee(service) {
...
function getDemoUsersList() {
    if (typeof window === "undefined")
        return Object.values(exports.DEMO_PRESET_USERS);
    try {
        ...
        return [...Object.values(exports.DEMO_PRESET_USERS), ...(Array.isArray(custom) ? custom : [])];
    }
    catch {
        return Object.values(exports.DEMO_PRESET_USERS);
    }
}
function saveRegisteredDemoUser(user) {
    if (typeof window === "undefined")
        return;
    try {
        const raw = window.localStorage.getItem("logmas.demo.users");
...
// ==========================================
async function handleMockApiRequest(config) {
    ...
    const respondError = (message, code = "REQUEST_FAILED", status = 400) => ({
        status,
    ...
    if (url.startsWith("/auth/me")) {
        ...
        if (!user) {
            user = exports.DEMO_PRESET_USERS.citizen;
        }
        return respond(user);
    }
    ...
    if (url.startsWith("/auth/google")) {
        const user = exports.DEMO_PRESET_USERS.citizen;
        return respond({
    ...
    if (url.startsWith("/auth/forgot-password") ||
    ...
    if (url === "/applications" || url.startsWith("/applications?")) {
        if (method === "GET") {
            let all = (0, applicationsStore_1.getLgaApplications)();
            // Light client-side filtering so dashboard filters stay functional.
            const q = (params.search || "").toString().toLowerCase();
            if (q) {
                all = all.filter((a) => (a.applicant && a.applicant.toLowerCase().includes(q)) ||
        ...
        if (method === "POST") {
            ...
            const form = data.formData && typeof data.formData === "object" ? data.formData : {};
            const serviceId = data.serviceId || "certificate_of_origin";
            const srv = (0, services_config_1.getServiceById)(serviceId) || services_config_1.DEFAULT_SERVICES[0];
            const fee = getEffectiveServiceFee(serviceId);
            const fullName = form.fullName || data.fullName || (typeof data.applicant === "string" ? data.applicant : "") || "";
            const createdApp = (0, applicationsStore_1.createLgaApplication)({
                serviceId,
            ...
            const inv = (0, store_1.createInvoice)({
            ...
            (0, applicationsStore_1.linkApplicationInvoice)(createdApp.id, inv.id, inv.reference);
            createdApp.invoiceId = inv.id;
            createdApp.invoiceNumber = inv.reference;
            inv.applicationId = createdApp.id;
            createdApp.applicationNumber =
                createdApp.applicationNumber || createdApp.applicationNo;
            ...
            return respond({
                ...
                invoice: {
                    ...
                    fieldOfficer: null,
                    qrData: "",
                    receipt: inv.status === "paid" ? { id: inv.receiptId, receiptNumber: inv.receiptNumber, verificationCode: inv.verificationCode, qrToken: inv.qrToken, issuedAt: inv.paidAt } : null,
                    permit: null,
                    virtualAccount: null,
                    payments: inv.status === "paid" ? [{ id: inv.paymentId || inv.id, amount: inv.amount, method: "online", status: "success", reference: inv.reference, confirmedAt: inv.paidAt, createdAt: inv.paidAt }] : [],
    ...
    if (adminAppAction) {
        ...
        const app = (0, applicationsStore_1.getLgaApplicationById)(appId);
        if (!app)
            return respond(null, 404);
        if (action === "under-review") {
            const updated = (0, applicationsStore_1.updateApplicationStatus)(appId, "Under Review", { name: "Council Admin", role: "lga_admin" }, { reviewNotes: data.notes || undefined });
            triggerSync();
            return respond(updated);
        }
        if (action === "approve") {
            if (app.paymentStatus !== "paid") {
                return respondError("This application has not been paid yet. Payment confirmation is required before the LGA can approve and issue the certificate.", "PAYMENT_REQUIRED");
            }
            const chairmanName = lga_config_1.LGA_CONFIG.leadership.chairman.name;
            ...
            const updated = (0, applicationsStore_1.updateApplicationStatus)(appId, "Approved", { name: "Council Admin", role: "lga_admin" }, {
                paymentStatus: "paid",
        ...
        const updated = (0, applicationsStore_1.updateApplicationStatus)(appId, "Rejected", { name: "Council Admin", role: "lga_admin" }, {
    ...
    if (invActionMatch) {
        ...
        if (inv) {
            const receipt = (0, store_1.markInvoicePaid)(inv.id, "online", "Demo Taxpayer", "citizen");
            ...
            const apps = (0, applicationsStore_1.getLgaApplications)();
            const matchedApp = apps.find((a) => (a.invoiceId && a.invoiceId === inv.id) ||
                (a.invoiceNumber && a.invoiceNumber === inv.reference) ||
                a.applicationNo === inv.reference ||
                a.id === inv.reference);
            if (matchedApp) {
                (0, applicationsStore_1.updateApplicationStatus)(matchedApp.id, "Payment Confirmed", {
                    name: "Demo Taxpayer",
                    role: "citizen",
                }, {
                    paymentStatus: "paid",
    ...
    if (invSingleMatch) {
        ...
        const s = (0, store_1.getStore)();
        const inv = s.invoices.find((i) => i.id === idOrRef || i.reference.toUpperCase() === idOrRef.toUpperCase());
        if (inv) {
            ...
            const linkedApp = (0, applicationsStore_1.getLgaApplications)().find((a) => a.id === inv.applicationId ||
                a.invoiceId === inv.id ||
            ...
            return respond({
                ...inv,
                invoiceNumber: inv.reference,
                applicationId: linkedApp?.id || inv.applicationId || null,
    ...
    if (url.startsWith("/payments/verify/")) {
        const ref = decodeURIComponent((url.split("/").pop() || "").split("?")[0]);
        const inv = (0, store_1.findInvoiceByRef)(ref);
        if (!inv) {
            return respond({
                status: "failed",
                paid: false,
        ...
        const app = (0, applicationsStore_1.getLgaApplications)().find((a) => (a.invoiceId && a.invoiceId === inv.id) ||
            (a.invoiceNumber && a.invoiceNumber === inv.reference));
        return respond({
    ...
    if (url === "/receipts" || url.startsWith("/receipts?")) {
        return respond((0, store_1.getStore)().receipts);
    }
    const receiptMatch = url.match(/^\/receipts\/([^/?]+)$/);
    if (receiptMatch) {
        const id = receiptMatch[1];
        const rec = (0, store_1.getStore)().receipts.find((r) => r.id === id || r.receiptNumber === id);
        if (rec)
            return respond(rec);
    }
    // ==========================================
    ...
    if (url === "/services" || url.startsWith("/services?")) {
        return respond(services_config_1.DEFAULT_SERVICES.map(serviceWithEffectiveFee));
    }
    const serviceSlugMatch = url.match(/^\/services\/([^/?]+)$/);
    if (serviceSlugMatch) {
        const slug = serviceSlugMatch[1];
        const s = (0, services_config_1.getServiceById)(slug) || services_config_1.DEFAULT_SERVICES[0];
        return respond(s);
    }
    // ==========================================
    ...
    if (url === "/certificates" || url.startsWith("/certificates?")) {
        ...
        const certs = apps.map((app) => ({
            ...
            issuedBy: {
                id: "usr_chairman_001",
                name: lga_config_1.LGA_CONFIG.leadership.chairman.name,
                role: lga_config_1.LGA_CONFIG.leadership.chairman.title,
            },
            application: {
                id: app.id,
                applicationNumber: app.applicationNo,
    ...
    if (url.startsWith("/public/certificates/") || url.startsWith("/certificates/verify/")) {
        ...
        const apps = (0, applicationsStore_1.getLgaApplications)();
        const foundApp = apps.find((a) => a.id === token ||
            a.applicationNo === token ||
            a.qrToken === token ||
            a.verificationCode === token) || apps[0];
        const cert = (0, apiPublicCertificate_1.transformApplicationToPublicCertificate)(foundApp, token);
        return respond(cert);
    }
    if (url.startsWith("/verify/")) {
        const code = url.split("/").pop() || "";
        const result = (0, store_1.findByQrOrCode)(code);
        return respond({
            valid: true,
            details: result || {
                issuedTo: "Dr. Babatunde Adeleke",
                service: "Certificate of Origin",
                issuedBy: lga_config_1.LGA_CONFIG.identity.councilName,
    ...
    if (url.startsWith("/reports/export/invoices")) {
        const s = (0, store_1.getStore)();
        return respond(s.invoices.map((i) => ({
            reference: i.reference,
            customerName: i.customerName,
            levyType: i.levyType,
            status: i.status,
    ...
    if (url.startsWith("/reports/export/receipts")) {
        const s = (0, store_1.getStore)();
        return respond(s.receipts.map((r) => ({
            receiptNumber: r.receiptNumber,
            customerName: r.customerName,
            levyType: r.levyType,
            paymentMethod: r.paymentMethod,
    ...
    if (url.startsWith("/treasurer/overview")) {
        ...
        return respond({
            ...
            revenueByService: services_config_1.DEFAULT_SERVICES.map((srv) => ({
                serviceId: srv.id,
                serviceName: srv.name,
                serviceCode: srv.id,
    ...
    if (url.startsWith("/treasurer/service-fees") || url.startsWith("/treasurer/fees")) {
        ...
        if (feeSubMatch) {
            ...
            const srv = services_config_1.DEFAULT_SERVICES.find((s) => s.id === srvId) || services_config_1.DEFAULT_SERVICES[0];
        ...
        return respond(services_config_1.DEFAULT_SERVICES.map((srv) => {
            const fee = getEffectiveServiceFee(srv.id);
            return {
                ...serviceWithEffectiveFee(srv),
                feeConfig: {
    ...
    if (url.startsWith("/field-officer")) {
        ...
        if (url.startsWith("/field-officer/businesses")) {
            ...
            return respond(s.customers.map((c) => ({
                id: c.id,
    ...
    if (url.startsWith("/lga/staff")) {
        ...
        return respond((s.officers || []).map((o) => ({
            id: o.id,
    ...
    if (url.startsWith("/customers")) {
        ...
        if (method === "POST") {
            ...
            (0, store_1.setStore)((prev) => ({ ...prev, customers: [newC, ...prev.customers] }));
            return respond(newC);
        }
    ...
    if (url.startsWith("/dashboard/overview") ||
        ...
        const scopedApps = (isCitizenLikeRole && currentUser && currentUser.id)
            ? apps.filter((a) => a.applicantId === currentUser.id || a.createdById === currentUser.id)
            : apps;
        const approvedAppCount = scopedApps.filter((a) => {
        ...
        const awaitingFormCount = scopedApps.filter((a) => {
            const st = String(a.status || "").toLowerCase();
            return st !== "approved" && st !== "completed" && st !== "rejected" && st !== "draft";
        }).length;
        const rejectedAppCount = scopedApps.filter((a) => String(a.status || "").toLowerCase() === "rejected").length;
    ...
    if (url.startsWith("/notifications")) {
        if (url.includes("/read-all")) {
            (0, store_1.setStore)((s) => ({
                ...s,
                notifications: s.notifications.map((n) => ({ ...n, read: true })),
            }));
            return respond({ message: "All notifications marked as read." });
        }
        const rawNotifs = (0, store_1.getStore)().notifications || [];
        const notifs = rawNotifs.map((n) => ({
            id: n.id,
            userId: n.userId || "usr_demo",
            title: n.title,
            message: n.message || n.body || "System update notification",
            type: n.type || "system",
    ...
    if (url.startsWith("/audit-logs") || url.startsWith("/activity-logs")) {
        return respond((0, store_1.getStore)().audits);
    }
    // ==========================================
